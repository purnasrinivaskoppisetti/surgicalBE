    ...
    async def create_product(
        ...
        try:
            ...
            if payload.sale_price > payload.mrp:
                raise HTTPException(status_code=400, detail="Sale price cannot be greater than MRP")
            ...
            for image in images:
                ...
                if len(contents) > MAX_IMAGE_SIZE:
                    raise HTTPException(status_code=400, detail=f"{image.filename} exceeds 3 MB limit")
                await image.seek(0)

            slug = slugify(payload.name)

            # Instantiating product with dimensions
            product = Product(
            ...
            await db.rollback()
            raise HTTPException(status_code=400, detail="Duplicate product data found")
    ...
    async def update_product(
        ...
        try:
            ...
            if images:
                ...
                await ProductImageRepository.delete_product_images(db, product.id)
