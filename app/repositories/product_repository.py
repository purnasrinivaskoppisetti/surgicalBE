...
class ProductRepository:
    ...
    @staticmethod
    async def get_by_id(db: AsyncSession, product_id: UUID):
    ...
    @staticmethod
    async def get_by_sku(db: AsyncSession, sku: str):
        result = await db.execute(
            select(Product)
            .where(
                Product.sku == sku,
    ...
    @staticmethod
    async def update(db: AsyncSession, product: Product):
        await db.commit()
        await db.refresh(product)
        return product
    ...
    @staticmethod
    async def update_product_rating(db: AsyncSession, product_id: UUID):
        result = await db.execute(
            select(
        ...
        product.rating = float(round(avg_rating or 0, 1))
